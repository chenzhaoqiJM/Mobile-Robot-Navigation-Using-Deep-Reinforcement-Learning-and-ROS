...
"""
2D激光雷达环境节点
基于原始environment.py修改，支持2D LaserScan数据
用于深度强化学习(DRL)导航训练
"""
...
from sensor_msgs.msg import LaserScan  # 使用LaserScan代替PointCloud2
...
class Environment2DLidar(Node):
    """2D激光雷达环境节点，为DRL提供所需服务。
    ...
    def __init__(self):
        super().__init__("gym_node_2d_lidar")
        ...
        self.get_logger().info("使用2D激光雷达 (LaserScan)")
        ...
        # 声明2D雷达话题参数（可通过launch文件配置）
        self.declare_parameter("laser_topic", "/front_laser/scan")
        self.laser_topic = (
    ...
    def terminate_session(self):
        """销毁节点并关闭rclpy"""
        self.get_logger().info("gym_node_2d_lidar 正在关闭...")
        self.destroy_node()
    ...
    def update_environment_state(self, laser_data):
        ...
        与3D点云处理相比，2D激光扫描的处理更加简单高效：
        - LaserScan已经是极坐标格式（角度+距离）
        - 不需要从笛卡尔坐标计算角度
        - 数据量更小，处理更快
        """
...
def main(args=None):
    ...
    # 创建环境节点
    environment = Environment2DLidar()
    ...
    try:
        ...
        environment.get_logger().info("gym_node_2d_lidar 正在关闭...")
